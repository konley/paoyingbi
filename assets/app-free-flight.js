(() => {
  ...
  function launch(strength = .55, horizontal = 0, options = {}) {
    ...
    const viewportScale = innerWidth < 600 ? 1.05 : 1.12;
    Body.setVelocity(coinBody, {
      x: Number.isFinite(options.vx) ? options.vx : horizontal * (2.4 + launchStrength * 2.6) * viewportScale,
      y: Number.isFinite(options.vy) ? options.vy : (-11.5 - launchStrength * 4.2) * viewportScale,
    });
  ...
  function beginReveal() {
    ...
    const duration = reducedMotion
      ? 260
      : clamp(520 + travel * 1.15 + remainingAngle / Math.PI * 220 + spinSpeed * 8, 680, 2600);
  ...
  function tick(now) {
    ...
    if (state === "dragging") {
      ...
      if (settled || age > 4200) beginReveal();
